...
# KooL OpenSuse-Hyprland uninstall script #
...
whiptail --title "OpenSuse-Hyprland KooL Dots Uninstall Script" --yesno \
...
# Function to remove selected packages on openSUSE
remove_packages() {
    local selected_packages_file=$1
    while read -r package; do
        # Check if the package is installed using zypper
        if zypper search --installed-only "$package" &> /dev/null; then
            echo "Removing package: $package"
            if ! sudo zypper remove -y "$package"; then
...
packages=(
    ...
    "kvantum-manager" "QT apps theming" "off"
    "kvantum-qt5" "QT apps theming" "off"
    "kvantum-qt6" "QT apps theming" "off"
    "kvantum-themes" "QT apps theming" "off"
    "libqt5-qtdeclarative-tools" "QT apps theming" "off"
    "libQt5QuickControls2-devel" "QT apps theming" "off"
    "qt6-declarative-imports" "QT apps theming" "off"
    "qt6-declarative-tools" "QT apps theming" "off"
    "mousepad" "simple text editor" "off"
    "mpv" "multi-media player" "off"
    "mpv-mpris" "mpv-plugin" "off"
    "NetworkManager-applet" "NetworkManager-applet" "off"
